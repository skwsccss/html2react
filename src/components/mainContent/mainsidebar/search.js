...
export default class SidebarSearch extends Component {
    ...
    toggleSearchMenu() {
        this.setState({ toggleSearchMenu: !this.state.toggleSearchMenu })
        console.log(this.state.toggleSearchMenu);
    }
    render() {
        var toggle = this.state.toggleSearchMenu;
        return (
            <div className="row align-items-center justify-content-between theme-light-bg no-gutter">
                <div className="col-12 search-bar">
                    ...
                    <span className="btn-group dropdown">
                        <span className="search-menu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Search Menu" onClick={this.toggleSearchMenu()}><i className="sprite search-menu-ico"></i></span>
